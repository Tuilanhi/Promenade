        ...
        .navigationDestination(isPresented: $navigateToConfirmPage) {
            if let selectedRouteId = selectedRouteId,
               let selectedRoute = routeOptions.first(where: { $0.id == selectedRouteId }) {
                ConfirmPageView(currentTime: Date().formattedTime(), destinationTime: selectedRoute.formattedDestinationTime, userCurrentLocation: sourceCoordinates, userPickup: destinationCoordinates)
            } else {
